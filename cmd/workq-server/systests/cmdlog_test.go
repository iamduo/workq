	...
	for i := 1; i <= n; i++ {
		...
		case 100:
			...
			fn := func(client *workq.Client) error {
				result, err := client.Result(j.ID, 20)
				if err != nil {
					return err
				}
		...
		case 300:
			...
			_, err := client.Lease([]string{name}, 20)
			if err != nil {
				return nil, err
			}
		...
		case 500:
			...
			fn := func(client *workq.Client) error {
				...
				if time.Now().After(created.Add(time.Duration(j.TTL) * time.Millisecond)) {
					if _, err := client.Lease([]string{name}, 20); err == nil {
				...
				job, err := client.Lease([]string{name}, 20)
				if err != nil {
					return err
				}
